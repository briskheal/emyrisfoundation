...
const HeroStatManager = ({ token }) => {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState(null);
  const [formData, setFormData] = useState({ id: '', value: '', label: '' });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/hero-stats?id=${id}`, {
        method: 'DELETE',
  ...
  return (
    <div className="admin-panel-section" style={{ display: 'block' }}>
      <h2 style={{ color: '#fff', marginBottom: '20px' }}>Manage HeroStats</h2>
      
      <div className="glass-card" style={{ padding: '20px', marginBottom: '30px' }}>
        <h3 style={{ color: '#15F5BA', marginBottom: '15px' }}>{editing ? 'Edit' : 'Add New'} HeroStat</h3>
        <form onSubmit={handleSave} style={{ display: 'grid', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
            <label style={{ color: '#fff', fontSize: '0.9rem' }}>Value</label>
            <input type="text" className="form-input" required value={formData.value || ''} onChange={e => setFormData({...formData, value: e.target.value})} style={{ background: 'rgba(255,255,255,0.05)', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', padding: '10px', borderRadius: '4px' }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
            ...
            {editing && <button type="button" className="btn btn-outline" onClick={() => { setEditing(null); setFormData({ id: '', value: '', label: '' }); }}>Cancel</button>}
          </div>
        </form>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', color: '#fff' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
              <th style={{ padding: '12px', textAlign: 'left', color: '#15F5BA' }}>ID</th><th style={{ padding: '12px', textAlign: 'left', color: '#15F5BA' }}>Value</th><th style={{ padding: '12px', textAlign: 'left', color: '#15F5BA' }}>Label</th>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <td style={{ padding: '12px' }}>{item.id}</td><td style={{ padding: '12px' }}>{item.value}</td><td style={{ padding: '12px' }}>{item.label}</td>
            ...
            {items.length === 0 && !loading && (
              <tr><td colSpan="4" style={{ padding: '20px', textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}>No items found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default HeroStatManager;
